...
def write_can():
    ...
    while not rospy.is_shutdown():
        ...
        can_msg = Message(arbitration_id=0x295, data=[drivemode, speed_left_L, speed_left_H, speed_right_L, speed_right_H, gear_target, 0x00, 0x00])
        # Send the message over CAN
        bock_can.send(can_msg)

        
    # Send request of deactivation
    deact_msg = Message(arbitration_id=0x195, data=[0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00])
